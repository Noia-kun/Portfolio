...
const ShapeGrid: React.FC<ShapeGridProps> = ({
  ...
  const colorsRef = useRef({ borderColor, hoverFillColor, voidColor });

  useEffect(() => {
    colorsRef.current = { borderColor, hoverFillColor, voidColor };
  }, [borderColor, hoverFillColor, voidColor]);
  ...
  useEffect(() => {
    ...
    const drawGrid = () => {
      ...
      const currentBorder = resolveColor(colorsRef.current.borderColor);
      const currentHover = resolveColor(colorsRef.current.hoverFillColor);
      const currentVoid = resolveColor(colorsRef.current.voidColor);
      ...
      if (isHex) {
        ...
        for (let col = -2; col < cols; col++) {
          for (let row = -2; row < rows; row++) {
            // Crisp 1px alignment via + 0.5 offset
            const sx = Math.floor(col * squareSize + offsetX) + 0.5;
            const sy = Math.floor(row * squareSize + offsetY) + 0.5;
      ...
      if (currentVoid !== "transparent") {
        ...
        gradient.addColorStop(1, currentVoid as string);
